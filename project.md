...
### 信贷网络的级联失效
每一个时期，随着信贷网络的增长和收入支出的变化；多个时期之后，有些个体就可能出现净资产变成负数，对应了资不抵债的违约。如果违约事件在信贷网络上传播，就对应了级联失效的发生。
级联失效过程：一个个体违约，它的资产负债表失效，对应的关联节点上面的资产/负债也随之失效。这个失效过程不断破坏这个信贷网络，直到结束。

要确定级联违约的阈值，就是连锁反应波及了多少个体的违约，才算是一次崩塌事件。
一旦发生了大面积的崩塌，才算是临界现象，这个时候的信贷规模，才是达到了临界信贷规模。

我们把问题简化，级联失效无论波及到多少个节点，都是在一期内发生的。换句话说，级联失效无论规模多么大，都是瞬间完成的，也就是整个级联失效需要在这一期内完成。
...
5. 执行 `period_end` 期末违约检查与级联失效
   ...
   - 违约条件由净资产判断，例如 `净资产 < 0`。
   - 如果出现违约节点，则在本 `period_end` 内完成级联失效分析。

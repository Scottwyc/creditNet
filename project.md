...
投资支出量 = 上一期的借贷量。
原始设想中的“计划总投资目标 = c * 上一期总收入”，在当前实现中具体操作化为每期计划执行的单位信贷 `time_step` 数：
`K_t = round(c * 上一期总收入)`。因此，当前 `c` 直接控制的是收入到计划信贷尝试批量的转换，不是实际投资支出比例。
实际成功借贷量不超过计划 `time_step` 数；当期成功借入量再作为期末投资支出目标，实际投资支出还会受到期末现金约束。
...
1. **基线 `period_end` 协议**：一次级联失效是一次 `period_end` 事件。一个 `period` 先执行本期计划的 `K_t` 个单位信贷尝试，再完成期末投资/消费支出、收入分配和净资产更新；只有在这个期末检查点，才判断是否发生违约和级联。
2. **timestep 对照 `timestep_settle_check` 协议**：一个宏观 `period` 仍先由 `K_t = round(c * Y_(t-1))` 给出计划微步数，但该期被拆成 `K_t` 个微观 credit `time_step`。每个 `time_step` 至多新增 1 单位信贷，随后立刻进行一次按 `1/K_t` 缩放的投资/消费支出结算、收入分配、违约检查和完整级联清算。若触发 avalanche，则暂停信贷增长，先把清算队列处理完，再进入下一个 `time_step`。
...
显式拓扑不是“初始化时已经存在的信贷暴露网络”。初始化时信贷暴露矩阵仍为空；`topology` 先给出一张无向的交易机会图，只规定哪些节点对可以在后续 credit `time_step` 中发生借贷。实际信贷网络是在运行过程中逐步增长出来的：每次成功借贷都会在机会图允许的节点对之间新增 1 单位有向暴露，方向由当次贷款人与借款人决定，重复发生时累加边权。`complete` 拓扑表示不施加额外机会边界。
...
重复上述实验 `M` 轮次，统计其中的失效规模与频率关系，分析是否有幂律关系。
...
### 已完成的 period/timestep 两种粒度实验讨论

当前文档已经纳入最新的两种事件观测粒度：`period_end` 基线协议和 `timestep_settle_check` 对照协议。两者的差异不是违约阈值或清算规则不同，而是流量结算与违约检查的采样尺度不同：

- `period_end` 协议把一个宏观期内的 `K_t` 个单位信贷尝试累积到期末，再统一做投资/消费支出、收入分配、违约检查和级联清算。
- `timestep_settle_check` 协议把同一宏观期拆成单位 credit `time_step`，每个微步后立即做按 `1/K_t` 缩放的微结算、收入分配、违约检查和完整清算。

已有完备场景扫描给出的结论应分开解释：

- **period_end 完备扫描**：覆盖本金初始化、支出机制、收入分配、增长规则、显式拓扑和 `c=0.10..0.80`。结果显示，随着 `c` 增大，期末批量结算下的大级联失效率显著上升，形成清楚的过载相图；但 SOC 快筛候选数为 0，因此它支持“级联失效/过载转变”，不支持严格 SOC。
- **timestep 对照扫描**：使用同一 8000 场景轴，在每个 credit `time_step` 后微结算和检查。快速全景 v2 设置为 `1 seed × 20 periods`、`max_period_length_steps=500`。结果显示，`4,539,260` 个 timestep avalanche 中没有达到 `10%N` 的大级联，最大事件规模为 `10/120`；严格 SOC 快筛候选数为 0，但存在 28 个“小尺度尾部/幂律样貌”候选。

因此，最新实验讨论的核心结论是：`period_end` 粒度下的大崩塌明显依赖期末批量结算窗口；当观测和结算推进到 timestep 粒度后，大级联被切碎为高频、小规模 avalanche，并出现一定的小尺度重尾/近幂律现象。这个结果说明 timestep 尺度值得继续做长时、多 seed、有限尺寸复核；但在当前证据下，它仍不能被直接提升为严格 SOC 证明。

### 已完成扫描的固定参数基线

不同场景扫描时，报告中的“场景”主要改变 `c`、本金分布、收入分配、增长规则、支出机制和拓扑机会图。为了使这些场景可比较，其他关键机制保持固定。

`period_end` 完备扫描的固定基线为：

| 固定项 | 取值 |
| --- | --- |
| 节点数 | `N=120` |
| 重复数 | 每个 scenario `3` 个 seed/run |
...
| 单位信贷 | 每个成功 `time_step` 新增 `1` 单位信贷暴露 |
| 违约阈值 | `default_threshold=0.0`，严格 `W_i<0` |
| 大级联阈值 | `collapse_threshold_fraction=0.10`，即 `N=120` 时至少 `12` 个节点 |
| 清算/恢复 | `wipe_defaulted_assets=True`，`continue_after_avalanche` |
| 检查粒度 | `period_end` |
| 会计校验 | `validate_accounting=True` |
| SW 重连概率 | `sw_rewire_probability=0.10` |

`timestep_settle_check` 对照扫描沿用相同场景轴和大多数固定机制，但快速全景 v2 的固定基线改为：

| 固定项 | 取值 |
| --- | --- |
| 节点数 | `N=120` |
| 重复数 | 每个 scenario `1` 个 seed/run |
| 单 run 长度 | `max_periods=20`，`max_time_steps=0` |
| 单 period 微步上限 | `max_period_length_steps=500` |
| 初始本金规模 | `mean_initial_money=20.0` |
| 初始参考收入 | `initial_income_per_capita=5.0`，即 `Y_0=600` |
| period 长度 | `K_t=round(c*Y_(t-1))`，实际微步数为 `min(K_t,500)` |
| 单位信贷 | 每个成功 `time_step` 新增 `1` 单位信贷暴露 |
...
| 会计校验 | 正式 v2 为加速设置 `validate_accounting=False`；smoke test 使用校验通过 |
| SW 重连概率 | `sw_rewire_probability=0.10` |

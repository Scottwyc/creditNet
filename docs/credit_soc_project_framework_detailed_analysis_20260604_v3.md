# 信贷网络自组织临界详细分析报告 v3：基于 project.md 框架

更新时间：2026-06-04 13:10 CST

## 1. 报告目的

本报告严格按照 `/data/WYC/class/creditNet/project.md` 的问题框架，对当前信贷网络自组织临界实验进行机制解释、结果分类和证据审计。

本版在旧版实验结果和图片基础上，进一步补充模型框架与变量口径审计。凡是报告中使用的“驱动强度”“时期长度”“信贷规模”“大崩塌”“临界事件”“传播代”“分支比”“稳态”“幂律”等术语，均在第 2 节给出可对应到代码字段的准确定义。
...
相较 v2，v3 新增了级联事件内部拆分、传播动态、长期状态诊断和严格尾部统计的统一变量字典，并明确区分旧版固定 `xmin=2` 的连续 Pareto 描述性 `alpha` 与第二阶段自动 `xmin` 的离散幂律 `alpha`。这些定义用于解释模型和证据边界；最终严格统计数值仍以第二阶段严格 SOC 报告和最终综合报告为准。
...
- 描述性 `alpha`：旧版固定 `xmin=2` 连续 Pareto 近似的尾部指数初筛值，不是严格幂律证明；
...
### 2.8.1 级联事件必须拆成“初始违约”和“传播新增”

一次 `period_end` 会先完成全系统投资/消费支出和收入重分配，再同时识别所有负净资产节点，最后才沿信贷暴露传播损失。因此，一个记录的 avalanche 不一定来自单一微观触发点：

| 量 | 定义 |
| --- | --- |
| `initial_default_count = I_0` | 流量结算后、网络传播开始前已经满足 `W_i < default_threshold` 的节点数 |
| `propagated_default_count = P` | 因清算减记贷款资产而新增的违约节点数，定义为 `collapse_size - initial_default_count` |
| `collapse_size = S` | 该事件最终处理和清算的不同节点总数，满足 `S = I_0 + P` |
| `propagation_share` | `P/S`，表示事件中真正由网络传播新增的比例 |
| `single_trigger_event` | `initial_default_count == 1` 的事件标签 |

`single_trigger_event` 也不等于外生单点冲击，因为初始违约是在全系统统一流量结算后产生的。即使固定 `K=1`，一次期末结算仍可能同时产生多个初始违约节点。

这一区分直接影响有限尺寸解释：如果 `collapse_size` 随 `N` 的增长主要来自 `initial_default_count` 的广延增长，就不能把总事件规模的近线性增长直接解释成传播过程接近临界。

### 2.8.2 传播动态变量

第二阶段动态分析按当前 FIFO 清算队列记录节点“最早被发现的传播代”。这些变量是清算算法诊断，不是额外的物理时间尺度：
...
| `propagation_depth` | `duration_generations - 1` | avalanche 前经历的 period 数 |
...
基线 FIFO 队列具有 sticky queue 语义：节点一旦在负净资产时入队，之后即使其他清算动作使其净资产恢复，也仍会被处理。因此 wave 和 duration 描述的是当前算法的发现结构，不能直接解释为客观同步传播。

### 2.8.3 长期依赖、重复违约与稳态诊断

| 变量 | 准确定义 |
| --- | --- |
| `avalanche_period_occupancy` | 观测窗口中发生 avalanche 的 period 数占比；基线每期最多记录一次 |
| `waiting_periods` | 同一 run 中相邻 avalanche 检测期索引之差 |
| `size_lag1_correlation` | 同一 run 内相邻事件规模对汇总后的 Pearson 相关，不跨 run 连接 |
| `unique_default_nodes` | 一个 run 内至少违约一次的不同节点数 |
| `total_default_occurrences` | 一个 run 内所有 avalanche size 之和，同一节点可跨期重复计数 |
| `repeat_default_occurrences` | `total_default_occurrences - unique_default_nodes` |
| `repeat_default_share` | `repeat_default_occurrences / total_default_occurrences` |
| `final_active_credit` | run 结束时仍未被清算的活跃信贷暴露总量 |

`occupancy`、等待时间、lag-1 和 Q1-Q4/early-late 窗口用于判断事件依赖和时间漂移。它们不是单项 SOC 判据：离散慢驱动 SOC 也可能在每次驱动后产生非零 avalanche。当前模型的稳态审计必须联合检查事件依赖、状态漂移、重复违约、活跃信贷、净资产 Gini、初始/传播违约组成以及尾部和有限尺寸结果。

### 2.8.4 严格尾部统计与旧版描述性统计的区别

| 统计量 | 准确定义 |
| --- | --- |
| 描述性 `alpha` | 本报告旧实验部分使用的固定 `xmin=2` 连续 Pareto 近似初筛值 |
| 严格离散幂律 `alpha` | 在自动选择 `xmin` 以上，对离散 avalanche size 最大似然估计的尾部指数 |
| `xmin` | 纳入尾部模型拟合的最小 avalanche size；严格分析通过最小化 KS 距离自动选择 |
| KS 距离 | 经验尾部 CDF 与拟合模型 CDF 的最大差异 |
| KS bootstrap p 值 | 按拟合模型生成合成数据，并对每个合成样本重新选择 `xmin` 后得到的拟合优度 |
| `R = log L_powerlaw - log L_alternative` | 幂律与 exponential/lognormal 等替代分布的对数似然差；`R<0` 表示替代分布更优 |
| Vuong p 值 | 非嵌套模型比较的描述性显著性；事件强相关时不能按 iid 最终检验解释 |
| bounded power law | 显式考虑 `collapse_size <= N` 的有限支持幂律模型 |
| cutoff / moment scaling | P99、最大规模、`E[S^2]/E[S]` 等随 `N` 的增长 |

“不拒绝幂律”不等于证明幂律，更不等于证明严格 SOC。严格 SOC 需要联合满足：可信尾部、可解释的有限尺寸标度、慢驱动/快 avalanche 分离、长期统计稳定，以及不依赖把外部控制参数精细调到狭窄点。
...
| `actual_recovery_amount` | 债务人受现金上限约束后实际支付并被债权人回收的金额，可能远低于目标回收额 |
| `external_reset_cash_flow` | reset 将节点现金恢复到初始值时产生的累计外部现金变化；reset 不满足原始封闭系统现金守恒 |

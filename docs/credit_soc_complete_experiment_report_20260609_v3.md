...
报告目标不是只列出扫描变量，而是把模型机制、固定参数基线、扫描轴、级联失效相图、SOC 判定、timestep 对照和动画可视化统一放在一个完整实验叙述中。v2 相比 v1 重点补充当前模型机制、关键变量口径、period/timestep 时间粒度差异、持续失败与 SOC 的区别，并纳入同参数 period/timestep 对照动画。v3 进一步补充 `period_end` 相图临界区域的细步长扫描，用来检验低风险区和持续失败区之间是否存在 period 粒度 SOC 候选带。

核心结论是：

- `period_end` 粒度下，系统存在清楚的级联失效过载相图；高 `c`、高支出、biased 收入和部分高连接/异质拓扑会显著放大级联。
- `period_end` 临界带细扫发现了非饱和 SOC 候选带：典型区域为 `uniform` 收入、`random` 增长、`complete` 拓扑下的 `high_both, c≈0.56..0.59` 和 `high_wealth, c≈0.74..0.78`。其中 3 个点通过自动 6/6 快筛，但仍应解释为“调参临界候选”，不是严格 SOC 证明。
...
- 因此，粗网格 period_end 扫描支持级联失效相图和过载转变，但没有在 `c=0.1` 步长下发现稳定 SOC 临界带。
...
## 7. period_end 临界带细扫与 SOC 候选

专门报告：`docs/credit_soc_period_critical_band_report_20260609_v1.md`

在粗网格 `c=0.1..0.8, step=0.1` 的 period 扫描中，自动 SOC 快筛候选为 0；但这并不排除低风险区和持续失败区之间存在很窄的临界候选带。因此追加两阶段细扫。
...
- 设计：从旧相图筛出 12 个非饱和转变候选 base scenario，在 `center±0.10` 内以 `0.025` 步长扫描。
- 规模：100 个场景、2400 个 run、441278 个 avalanche/event。
- 结果：自动快筛出现 18 个候选，说明粗网格确实会漏掉窄临界区域。
...
- 设计：聚焦第一阶段最稳健的非饱和候选族，固定 `uniform` 收入、`random` 增长、`complete` 拓扑；本金分布取 `equal/normal/pareto`。
- `high_both=(a=0.04,b=0.40)` 扫描 `c=0.450..0.600`，步长 `0.005`。
- `high_wealth=(a=0.04,b=0.20)` 扫描 `c=0.675..0.800`，步长 `0.005`。
...
第二阶段使用三位小数 `c` 重建 `scenario_id` 后分析。原 `run_comprehensive_scenario_scan.py` 的两位小数场景 ID 已修正为三位小数，避免后续细步长扫描折叠相邻 `c`。

![period 临界带 SOC 快筛相图](../results/credit_soc_period_critical_stage2_exact_analysis_20260609_v1/global_soc_screen_phase_map.png)

![period 临界带 c-CCDF](../results/credit_soc_period_critical_stage2_exact_analysis_20260609_v1/ccdf_by_c.png)

![period 临界带候选 CCDF](../results/credit_soc_period_critical_stage2_exact_analysis_20260609_v1/top_candidate_ccdf_grid.png)
...
- 这说明 period 粒度下确实存在临界候选带，而不是只有低风险区和持续失败区两个粗相。
- 候选带具有连续性：`high_both` 约在 `c≈0.56..0.59`，`high_wealth` 约在 `c≈0.74..0.78`。
- 候选区 event occupancy 尚未进入完全持续失败，约为 `0.56..0.80`；传播新增违约占比约 `0.40..0.45`，说明不是纯同步初始违约。
- 但这些候选仍靠外部参数 `c` 的窄带调节，event occupancy 已接近过载边界，且最大事件规模只有 `25..33/120`。因此当前只能提升为“period 粒度 SOC 候选/调参临界带”，不能提升为严格 SOC。
...
综合 period 粗扫、period 临界带细扫、timestep 对照和动画可视化，结论如下：

1. **级联失效相图成立**：`period_end` 粒度下，系统随 `c`、支出强度、收入分配偏置和本金/拓扑结构出现清晰的过载转变。
2. **period_end 大崩塌有批量结算依赖**：期末统一结算会聚合同步初始违约和传播清算，显著放大单次事件规模。
3. **period 临界带确实存在候选**：细扫在 `uniform` 收入、`random` 增长、`complete` 拓扑下发现 `high_both, c≈0.56..0.59` 和 `high_wealth, c≈0.74..0.78` 两条非饱和候选带；其中 3 个点达到自动 SOC 快筛 6/6。
4. **这些候选还不是严格 SOC**：候选仍依赖窄 `c` 调节，event occupancy 接近 `0.8`，最大事件规模约 `25..33/120`，尚未证明有限尺寸缩放和无调参自组织。
5. **timestep 粒度下大级联消失**：同一 8000 场景轴改为微步结算后，没有任何场景达到 10%N 大级联阈值。
6. **timestep 下有小尺度重尾/近幂律现象**：部分场景在小 avalanche 范围内具有尾部候选，但尺度范围不足，不能提升为严格 SOC。
7. **period 的持续失败不是临界态证明**：高风险持续失败区更像超临界过载态或活动饱和区，不能直接等同于 SOC 的“临界态保持”。
8. **最终判定**：当前最稳妥表述是“period 粒度存在值得继续验证的 SOC 候选临界带；timestep 粒度存在小尺度重尾；严格 SOC 尚未被证明”。
...
- period 临界带细扫将重复数提高到每场景 24 个 seed，并把 `c` 步长压到 `0.005`，但仍固定 `N=120`，尚未做有限尺寸缩放。
- `timestep` 对照是快速全景 v2：每 scenario 1 个 seed、20 periods、单期最多 500 微步。
- 动画是小规模代表性重跑，用于机制解释，不是新增严格统计证明。

后续若要继续检验 SOC，应优先做：

1. 对 period 候选带做 `N=60/120/240/480` 的有限尺寸缩放，检查 cutoff 是否随 `N` 系统增长。
2. 在 `c≈0.59` 和 `c≈0.77..0.78` 附近做更长时模拟和更多 seed，检验平稳性和参数邻域鲁棒性。
3. 对 28 个 timestep 小尺度尾部候选做多 seed、长时、无上限或更高 `max_period_length_steps` 复核。
4. 明确拆分 `initial_default_count`、`propagated_default_count` 和 repeated defaults。
5. 做 `permanent_exit`、`reset`、`recovery_delay` 等恢复机制消融。
6. 比较 period_end、固定批次结算、timestep 结算在同一强度下的机制鲁棒性。

在当前证据下，最稳妥的表述是：该信贷网络模型存在机制敏感的级联失效和过载转变；period 粒度存在窄临界候选带；timestep 尺度出现小 avalanche 重尾现象；但严格自组织临界尚未被证明。

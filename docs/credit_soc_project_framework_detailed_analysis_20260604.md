# 信贷网络自组织临界详细分析报告：基于 project.md 框架

更新时间：2026-06-04 12:14:42 CST
...
| 消费支出 | `a * max(net_worth, 0) + b * last_income`，随机舍入并受现金约束 | 当前固定 `a=0.02,b=0.20` | 已实现，尚未系统扫描 a/b |
...
| 稀疏/密集、SW、BA | 显式固定拓扑或 SW/BA 拓扑 | 尚无 | 未实现 |
| 还款和债务消失 | 正常还款、到期、利息 | 尚无 | 未实现 |
| 严格幂律检验 | 自动 xmin、KS、似然比 | 尚无 | 未实现 |
...
| --- | --- | --- | --- |
| `N` / `n_nodes` | 节点数 | 200 | 是 |
| `M` / `runs` | 每个场景独立重复 run 数 | 20 或 60 | 是，属于实验重复数 |
...
| `c` / `investment_income_propensity` | 上一期总收入转化为下一期计划信贷 time_step 数的比例 | 0.02、0.05、0.10、0.20 | 仅收入内生 `K_t` 实验生效 |
...
| `initial_income_per_capita` | 第一期开始前的每人参考收入 | 5 | 是，但不注入现金 |
...
| `max_time_steps` | 单个 run 的累计尝试步数上限 | 0，表示不单独限制 | 是 |
...
因此，`c` 是“上一期全系统总收入转化为下一期计划信贷尝试步数的比例”，是无量纲参数。它：
...
需要特别说明：由于当前模型只在 `period_end` 检查违约，增大 `c` 不仅增加信贷驱动量，也增加两次违约检查之间的信贷批量。因此报告中的“`c` 效应”同时包含：

1. 更强的信贷增长驱动；
2. 更长的期内累积区间；
3. 在统一检查前积累更多潜在脆弱暴露。

若要把“纯驱动强度”与“违约检查频率”分离，后续需要在每个 time_step 后检查违约，或固定检查间隔并单独改变成功发放概率。
...
- `c=0.20` 正好对应 `project.md` 建议的初始值 `1/5`，说明该参数不是温和基线，而是当前模型中的高风险驱动强度。
...
图4-1显示，`c=0.02-0.10` 区间尚未出现达到 10% 节点阈值的大崩塌，而 `c=0.20` 下 lognormal 和 pareto 本金分布的大崩塌 run 率明显跃升，说明驱动强度附近存在非线性转变。
...
需要明确：这属于内生信贷边选择机制，不等同于显式 ER、BA 或 SW 固定拓扑。`project.md` 中提出的 SW/BA 对照尚未实现。
...
- 显式 ER、BA、SW 底层拓扑；
- 稀疏度和平均度控制；
- 聚类系数、路径长度、度异质性与临界规模的回归分析；
- 将网络结构统计量写入每个 avalanche 事件。
...
- `c` 同时改变信贷驱动批量和两次违约检查之间的间隔，尚未与检查频率效应分离。
...
4. **当前批量驱动与 period_end 检查机制下存在转变区。** 固定 K 在 `K=20` 左右开始出现稀有大 avalanche；收入内生 K 在 `c=0.20` 下出现稳定大 avalanche，但 `c` 的纯驱动效应尚未与检查间隔效应分离。
